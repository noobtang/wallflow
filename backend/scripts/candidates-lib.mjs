/**
 * 取材共用库(#10 扩库 + 2026-08-15 每周候选清单): 从 Wikimedia Commons 白名单来源
 * 抓取候选壁纸元数据。逻辑从 fetch-candidates.mjs 抽取,供 CLI(直接跑)与
 * weekly-candidates.mjs(汇总报告)复用。
 *
 * 来源策略(2026-08-11 实测):
 *   - 政府图库分类(PD 占比极高): PD NASA / USFWS / NOAA 等
 *   - 结构化数据搜索(haswbstatement:P275= 精确许可): CC0 / CC BY 4.0
 * 过滤: 许可 ∈ 白名单(CC0/CC BY/Public domain)、横向(w>h)、宽 ≥ 2000、尺寸 ≤ 20000。
...
export function isWhitelisted(lic) {
  if (!lic) return false;
  const s = lic.trim();
...
/** 抓取单个来源,返回通过过滤的候选数组(未落盘) */
export async function fetchSource(src, limit) {
  const params = {
    action: 'query',
...
/** 读取 data/candidates.json(不存在/损坏 → []) */
export function readCandidatesFile() {
  const p = path.join(ROOT, 'data', 'candidates.json');
  try {
...
/** 全量去重(imageUrl 相同),按源稳定排序;返回写盘前的候选数组 */
export function dedupeCandidates(all) {
  return all
