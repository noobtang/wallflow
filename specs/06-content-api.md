...
前端主数据源 — 首页信息流、分类过滤、详情页、相似推荐全部由此 API 提供。ENG-PLAN.md 决策: 元数据读缓存、keyset 分页、**图片由国内 COS 签名直链分发(#8),无上游依赖**。
...
实现 4 组只读内容路由。**注意: 返回的图片 URL 为 COS 签名直链(见 #8),短时效(~1h),前端直接用于加载/下载。**

### Implementation Details

| 路由 | 说明 | 参数 | 分页 |
|------|------|------|------|
| `GET /wallpapers` | 信息流/分类 | `category?`, `cursor?` | keyset(created_at, id) |
...
5. 返回的图片 URL 全部为 COS 域名签名直链,无任何海外上游直链
