...
- [x] 图片热链合规(2026-08-15 调研): 运行时不对上游热链 — 导入时一次性镜像到 COS(私有读+签名直链),天然规避改图/删图/限流;合规义务已全部落地(CC BY 署名+许可链接+修改声明、CC0 可无署名、禁 hot spider、导入限速);完整 primary source 对照见 `data/README.md`「图片再分发合规」节
